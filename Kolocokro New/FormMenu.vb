    ...
    Private Sub bwMain_DoWork(sender As Object, e As DoWorkEventArgs) Handles bwMain.DoWork
        ...
        If Not isPlaying Then
            ...
            For loopIndex As Integer = 0 To loopCount
                    ...
                    For Each row As DataGridViewRow In dgvStatus.Rows
                        ...
                        If row.Cells("SeleniumCommand").Value IsNot Nothing AndAlso row.Cells("ValueOrURL").Value IsNot Nothing Then
                            ...
                            Select Case seleniumCommand
                                Case "Read EXCEL"
                                    dataExcel = bot.ReadExcel(valueOrURL, value)
                                    If dataExcel IsNot Nothing Then
                                        tbLogs.Invoke(Sub() tbLogs.AppendText("Read EXCEL Berhasil" + vbNewLine))
                                        output = "Selesai"
                                    Else
                                        tbLogs.Invoke(Sub() tbLogs.AppendText("Read EXCEL Error" + vbNewLine))
                                        output = "Error"
                                    End If
                                    Exit Select
                                Case "Read CSV"
                            ...
                            'Error (DropDown)
                                ...
                                Case "Open"
                                    If valueOrURL.Contains("DataCSV") Then
                                        ...
                                        Else
                                        End If
                                    ElseIf valueOrURL.Contains("$data") Then

                                        If loopIndex > 0 Then
                                            ' Loop sampai tidak ada lagi kemunculan "$data" dalam string
                                            Dim index As Integer = valueOrURL.IndexOf("$data")
                                            While index <> -1
                                                ' Dapatkan indeks awal dan akhir dari kata $data
                                                Dim startIndex As Integer = valueOrURL.IndexOf("$data{[", index)
                                                Dim endIndex As Integer = valueOrURL.IndexOf("]}", startIndex)
                                                Dim colCSV = GetStringBetween(valueOrURL, "][", "]")

                                                ' Jika kata $data ditemukan, hapus bagian belakangnya
                                                If startIndex <> -1 AndAlso endIndex <> -1 Then
                                                    valueOrURL = valueOrURL.Remove(startIndex + 5, endIndex - startIndex - 3)
                                                End If

                                                ' Ambil nilai dari data CSV berdasarkan indeks baris dan kolom
                                                Dim csvValue = dataCSV(loopIndex)(colCSV)

                                                ' Ganti satu kemunculan $data dengan nilai dari data CSV
                                                Dim replaceIndex = valueOrURL.IndexOf("$data", index)
                                                valueOrURL = valueOrURL.Remove(replaceIndex, 5).Insert(replaceIndex, csvValue)

                                                ' Cari kemunculan "$data" berikutnya
                                                index = valueOrURL.IndexOf("$data", index + 1)
                                            End While
                                        Else
                                            ' Loop sampai tidak ada lagi kemunculan "$data" dalam string
                                            Dim index As Integer = valueOrURL.IndexOf("$data")
                                            While index <> -1
                                                ...
                                                ' Dapatkan indeks baris dan kolom dari CSV
                                                Dim rowCSV = GetStringBetween(valueOrURL, "[", "]")
                                                Dim colCSV = GetStringBetween(valueOrURL, "][", "]")
                                                ...
                                                ' Ambil nilai dari data CSV berdasarkan indeks baris dan kolom
                                                Dim csvValue = dataCSV(rowCSV)(colCSV)
                                            ...
                                            End While
                                        End If
                                        ' Lakukan operasi selanjutnya dengan URL yang telah dimodifikasi
                ...
                tbLoop.Invoke(Sub()
                                  tbLoop.Clear() ' Menghapus teks yang ada
                                  tbLoop.AppendText(loopIndex.ToString()) ' Menampilkan nilai baru
                              End Sub)
            Next
